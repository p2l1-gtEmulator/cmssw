...
static constexpr std::array<std::tuple<const char*, std::size_t, std::size_t>, 26> OUTPUT_CHANNELS_VU9P{
    {{"GTTPromptJets", 2, 6},
...
static constexpr std::array<std::tuple<const char*, std::size_t, std::size_t>, 26> OUTPUT_CHANNELS_VU13P{
    {{"GTTPromptJets", 2, 6},
     {"GTTDisplacedJets", 6, 10},
     {"GTTPromptHtSum", 10, 11},
     {"GTTDisplacedHtSum", 11, 12},
...
private:
  ...
  const edm::EDGetTokenT<P2GTCandidateCollection> cl2JetsToken_;
  ...
  const std::array<std::tuple<const char*, std::size_t, std::size_t>, 26> outputChannelDef_;
...
L1GTOutputObjectWriter::L1GTOutputObjectWriter(const edm::ParameterSet& config)
    : gctNonIsoEgToken_(consumes<P2GTCandidateCollection>(config.getParameter<edm::InputTag>("GCTNonIsoEg"))),
      ...
      cl2JetsToken_(consumes<P2GTCandidateCollection>(config.getParameter<edm::InputTag>("CL2Jets"))),
...
void L1GTOutputObjectWriter::analyze(const edm::Event& event, const edm::EventSetup&) {
  ...
  outputObjects.emplace("CL2Jets", std::move(fillCollection<L1TGT_CL2_Jet>(event, cl2JetsToken_)));
